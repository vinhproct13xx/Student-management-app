...
namespace GUI
{
    public partial class frmNhapDiem : Form
    {
        ...
        void getDiemQuyDinh()
        {
            ...
            dtoQuyDinh.DiemMax = int.Parse(dt.Rows[0][4].ToString());
            dtoQuyDinh.DiemMin = int.Parse(dt.Rows[0][5].ToString());
        }
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            ...
            for (int i=0; i<= dtoBangDiem.Dem - 1; i++)
            {
                ...
                {
                    MessageBox.Show("Vui lòng nhập số từ " + dtoQuyDinh.DiemMin+ " đến " + dtoQuyDinh.DiemMax + " cho học sinh có mã : " + dgvNhapDiem.Rows[i].Cells[0].Value.ToString());
                        return;
                }
                else if (dgvNhapDiem.Rows[i].Cells[2].Value.ToString() == "")
                {
                    MessageBox.Show("Vui lòng nhập điểm cho học sinh có mã: " + dgvNhapDiem.Rows[i].Cells[0].Value.ToString());
                    return;
                    i--;
                }
                try
                ...
                { }
                if (dtoBangDiem.Diem <dtoQuyDinh.DiemMin || dtoBangDiem.Diem > dtoQuyDinh.DiemMax)
                {
                    MessageBox.Show("Vui lòng nhập điểm từ " + dtoQuyDinh.DiemMin + " đến " + dtoQuyDinh.DiemMax + " cho học sinh có mã : " + dgvNhapDiem.Rows[i].Cells[0].Value.ToString());
                    return;
                    i--;
                }
                else
